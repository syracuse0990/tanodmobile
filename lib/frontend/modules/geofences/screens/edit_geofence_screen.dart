...
class _EditGeofenceScreenState extends State<EditGeofenceScreen> {
  ...
  Future<void> _loadData() async {
    ...
    await provider.fetchAvailableDevices();
    if (mounted) setState(() => _devicesLoaded = true);

    final geofence = await provider.fetchGeofenceDetail(widget.geofenceId);
    if (geofence != null && mounted) {
      setState(() {
        _nameController.text = geofence.name;
        _shape = geofence.shape;
        _alertOn = geofence.alertOn;

        if (geofence.isCircle) {
          if (geofence.centerLat != null && geofence.centerLng != null) {
            _circleCenter = LatLng(geofence.centerLat!, geofence.centerLng!);
          }
          _circleRadius = geofence.radius ?? 500;
        } else if (geofence.isPolygon && geofence.coordinates != null) {
          _polygonPoints.addAll(
            geofence.coordinates!.map((c) => LatLng(c.lat, c.lng)),
          );
        }

        _selectedDeviceIds.addAll(geofence.devices.map((d) => d.id));
        _initialLoaded = true;
      });
    }
  }
  ...
  /// Area currently covered by the shape being edited, in square metres.
  ...
  /// Boundary length of the shape being edited, in metres.
  ...
  Future<void> _submit() async {
    ...
    final provider = context.read<GeoFenceProvider>();
    final success = await provider.updateGeofence(
    ...
    if (success) {
      AppToast.show('Geofence updated', type: ToastType.success);
      await provider.fetchGeofences();
      if (mounted) context.pop();
    } else {
      AppToast.show(
        provider.error ?? 'Failed to update geofence',
  ...
  LatLng get _initialCenter {
    ...
    if (_polygonPoints.isNotEmpty) {
      final lats = _polygonPoints.map((p) => p.latitude);
      final lngs = _polygonPoints.map((p) => p.longitude);
      return LatLng(
          ...
          Expanded(
            child: ListView(
              ...
              children: [
                ...
                // Live measurement of the zone being edited
                const SizedBox(height: 12),
                GeofenceAreaSummary(
