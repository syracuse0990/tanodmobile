...
class TpsTicketDetailScreen extends StatefulWidget {
  const TpsTicketDetailScreen({super.key, required this.ticketId});
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          TextButton(
            ...
            child:
                const Text('Cancel', style: TextStyle(color: AppColors.mutedInk)),
          ),
          TextButton(
            onPressed: () async {
              Navigator.pop(ctx);
              ...
              if (mounted) {
                if (success) {
                  ...
                  AppToast.show('Failed to close ticket',
                      ...
                      children: [
                        ...
                        if (ticket.status == 'resolved' ||
                          ...
                          if (ticket.resolutionPhotoUrl != null) ...[
                            ...
                            GestureDetector(
                              onTap: () => _showFullImage(
                                  context, ticket.resolutionPhotoUrl!),
                              child: ClipRRect(
                                borderRadius: BorderRadius.circular(14),
                                child: Image.network(
...
class _AssistanceSheetState extends State<_AssistanceSheet> {
  ...
  Future<void> _submit() async {
    ...
    if (msg.isEmpty) {
      AppToast.show('Please describe what assistance you need',
          type: ToastType.error);
      return;
    }
      ...
      child: Column(
        children: [
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        const Text(
                          ...
                          style: TextStyle(
                              fontSize: 14, color: AppColors.mutedInk),
                        ),
                      ],
                    ),
                  ),
          ...
          Padding(
            ...
            child: _ChatInput(
                ticketId: ticketId, channelName: channelName),
          ),
        ],
      ),
    );
...
class _ChatInputState extends State<_ChatInput> {
  ...
  void _onTextChanged() {
    ...
    context.read<RealtimeProvider>().triggerClientEvent(
      widget.channelName,
      'client-typing',
      {'name': user?.name ?? 'Someone', 'role': 'TPS'},
    );
  ...
  Future<void> _send() async {
    ...
    final success = await context.read<TpsProvider>().addComment(
          ticketId: widget.ticketId,
              ...
              SizedBox(
                ...
                child: _sending
                    ? const Padding(
                        ...
                        child: CircularProgressIndicator(
                            color: AppColors.forest, strokeWidth: 2.5),
                      )
                    : IconButton(
                        onPressed: _send,
...
class _TypingIndicatorState extends State<_TypingIndicator>
  ...
  Widget build(BuildContext context) {
    final label =
        '${widget.userRole ?? ''} ${widget.userName} is typing...'.trimLeft();
